...
namespace Assets.MaterializeFX.Scripts
{
    internal static class MfxMaterialUtil
    {
        ...
        private const string EdgeColorPropName = "s_EdgeColor";
        ...
        public static void ReplaceRenderersMaterials(Material mfxTemplateMaterial, GameObject targetObject, MfxShaderType mfxShaderType, bool editorMode)
        {
            ...
            foreach (var targetRenderer in renderers)
            {
                ...
                var newMaterials = ReplaceMaterialsToMfx(mfxTemplateMaterial, targetRendererMaterials, mfxShaderType, editorMode);
                targetRenderer.sharedMaterials = newMaterials.ToArray();
            }
        }

        public static List<Material> ReplaceMaterialsToMfx(Material mfxTemplateMaterial, Material[] targetRendererMaterials, MfxShaderType mfxShaderType, bool editorMode)
        {
            ...
            foreach (var targetRendererMaterial in targetRendererMaterials)
            {
                ...
                var shaderName = mfxShaderType.GetShaderName();
                var newMaterial = new Material(targetRendererMaterial)
                {
                    shader = Shader.Find(shaderName)
                };

                CopyMfxProperties(mfxTemplateMaterial, newMaterial);
        ...
        private static void CopyMfxProperties(Material mfxTemplateMaterial, Material emptyMfxMaterial)
        {
            ...
            CopyPropertyToMaterial<float>(mfxTemplateMaterial, emptyMfxMaterial, FringeSizePropName, FringeSizePropName);
            CopyPropertyToMaterial<float>(mfxTemplateMaterial, emptyMfxMaterial, FringeAmountPropName, FringeAmountPropName);
            CopyPropertyToMaterial<Texture>(mfxTemplateMaterial, emptyMfxMaterial, FringeRampMapPropName, FringeRampMapPropName);
